...
export const usePicGoUpload = () => {
  ...
  const upload = async (file: Asset) => {
    ...
    formData.append('env', new Envs().get('APP_ENV') || 'sit');
    setUploading(true);
    xhr.open('POST', `${host}/upload/cloud`);
    xhr.setRequestHeader('token', useCaches.getState().token);
    ...
    xhr.upload.onprogress = e => {
      if (e.lengthComputable) {
        ...
        console.log(`上传进度: ${e.loaded}/${e.total} -> ${percent}%`);
        setProgress(percent);
      }
    };
  ...
  return {uploading, upload, picGo, progress};
};
